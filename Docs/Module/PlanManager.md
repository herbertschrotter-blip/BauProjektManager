...
**Status:** Schema v1.0 war implementiert seit v0.25.15. **Schema v2.0 DDL implementiert in BPM-109.01 (v0.28.55)** — die Tabellen werden von `PlanManagerDatabase.EnsureTables()` erzeugt. **Domain Models + Repository-Primitive in BPM-109.02 (v0.28.57)** (`PlanArchiveModels.cs` + additive `ResolveOrCreateDocument`/`InsertRevision`/`InsertSegment`/`GetCurrentRevisionLookup` etc.). Die alten Cache-Methoden + Import-Pipeline bleiben Fail-Fast bis **BPM-109.03** (Verdrahtung). Drei-Ebenen-Modell + Plan-Archiv-Persistenz als Foundation Slice in BPM-109 — siehe **ADR-058** + **ADR-058-Addendum** und **DB-SCHEMA.md Kap. 6.7** für die vollständige v2.0-Definition mit `plan_documents` (NEU), `plan_revisions` (umgebaut mit FK + Zeitstempeln), `plan_document_segments`, `plan_revision_events`, `plan_context_links` und `building_part_aliases`.
