...
| CGR-2026-08-27-bpm-architektur | Gesamtauswertung-Review: ChatGPTs 12-Diagramm-Analyse (Recovery-/Journal-Befunde, Import-Pipeline, Dateibrowser) vs. beschlossener Rahmen ADR-058–061 + BPM-109/111/112/113 | r1–r3 | ✅ **Abgeschlossen** | ChatGPT-Share „Softwareentwicklungsdiagramme" + Claude-Verifikation (v0.28.120) | **Beidseitiges Sign-off r3.** Neuer Task „Import-Transaktions-Härtung": Slices H0 (Alt-Import-Cutover) + T0–T8 (BPM-112 Slice 3 = T1), Start nach BPM-111.06. 11 Invarianten (Vorab-Journalisierung, completed-in-Transaction, gemeinsamer Apply-Pfad Import/Recovery, Undo nur nach vollem Disk-Reverse, skipDuplicate journalisiert/recovery-fähig/nicht undo-bar, destination_path nullable via DB-Reset) + 15 Akzeptanzkriterien. Neue verifizierte Befunde: Radial-Confirm über ungehärteten Executor; ImportUndoService rollt DB nach Disk-Fehlern bedingungslos zurück; ADR-061 P5 beschlossen aber nicht umgesetzt. Für V1 gestrichen: Skip-only/IsConflict/Preview-Ausbau/LearnIndex-Lernen. Extern offen: ID-basierter document_key = BPM-111-Abnahmepunkt. ChatGPTs 🔴-Befunde (5×) alle code-verifiziert; Diagramm-01–09-Nachlieferung ohne übersehenen V1-Blocker. **Resultiert in ADR-064 + ClickUp BPM-120** (Invarianten in ADR, 15 AKs im Ticket) |
| CGR-2026-08-27-plan-erkennung | Lernende Planerkennung: Profile als lernendes System — ChatGPTs Hybrid-Vorschlag (Regeln→Similarity→ML.NET/Embeddings/LLM→Confidence-Gate) vs. deterministisches Pattern Mining (L0–L3: Deterministik→Regeln→Evidenz-Vorschläge→Rule Mining) | r1–r3 | ✅ **Abgeschlossen** | ChatGPT-Konzept „Lernende Planerkennung" → Claude-Review-Position | **Beidseitiges Sign-off r3.** Lernende Erkennung = erklärbare, hierarchisch gescopte **Assistenz innerhalb der ADR-059-Grenze** (nur deterministisches Bestandsmatching entscheidet; alles Gelernte `AutoSuggested`). Kern: bestätigte Roh-Evidenz → nachvollziehbares Rule Mining → explizite Regeln + Aliasse; **kein ML.NET/Embeddings/LLM im Importpfad**. Evidenz-Backoff L2a (Projekt × `DocumentTypeId`) → L2b (explizite `profileLineageId`-Familie) → L2c (kuratierte FORMEN), keine Score-Fusion, Veto-Regel (unter-schwellige lokale Gegen-Evidenz blockiert Familien-Fallback), WERTE lokal / ROLLEN familienfähig / FORMEN global, ID-Auflösung immer lokal. **Dokumenttyp-Zielbild:** `document_types` = fachliche Typ-Wahrheit (Ring 1 + Erkennungs-Tab als View), `RecognitionProfile` = optionale Erkennungs-Konfiguration 0..1, kein leeres Profil-JSON, Löschen asymmetrisch, Lernen vor dem ersten Profil. **r3-Schärfungen:** `document_types.is_active` (nicht `is_deleted` missbrauchen) · PatternTemplate-Identität via Lineage (nicht projektlokale `DocumentTypeId`) · Tokenization-Bootstrap (Roh-Evidenz statt Token-Snapshots, Reparse via `FileNameParser`) · Mining nie Teil der Import-Transaktion (event-invalidiert, UI-demand-berechnet) · Fingerprint kanonisch escaped · UI-Begriff „Erkennung". Roadmap V1→A→B→C1(Alias)→C2(Lineage)→D(Lexikon)→ML nur bei gemessenem Bedarf. **Resultiert in ADR-065 + ClickUp-Task „Lernende Planerkennung".** |
