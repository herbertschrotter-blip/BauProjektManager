...
| CGR-2026-08-27-plan-erkennung | Lernende Planerkennung: Profile als lernendes System — ChatGPTs Hybrid-Vorschlag (Regeln→Similarity→ML.NET/Embeddings/LLM→Confidence-Gate) vs. deterministisches Pattern Mining (L0–L3: Deterministik→Regeln→Evidenz-Vorschläge→Rule Mining) | r1–r3 | Runde 3 (Konkretisierung + Abschluss) offen | ChatGPT-Konzept „Lernende Planerkennung" → Claude-Review-Position | **r2: ChatGPT-SIGN-OFF** + 2 Schärfungen übernommen (Veto-Regel: positive Schwelle ≠ Veto-Schwelle, unter-schwellige lokale Gegen-Evidenz blockiert Familien-Vorschlag · `ExactToken`-Grenzen aus profilabhängiger `TokenizationConfig`, nicht hardcodiert) + Dokumenttyp-Zielbild aus Herberts Zusatzfrage (beide Rückfragen bejaht): `document_types` = fachliches Hauptobjekt für Ring 1 + Profil-Tab (View), `RecognitionProfile` = optionale Erkennungs-Konfiguration 0..1, kein leeres Profil-JSON bei „+ Neu", Löschen asymmetrisch, L2a = Projekt × DocumentTypeId (Lernen vor dem ersten Profil). **r1 Konsens:** Lernen nur als Assist (ADR-059-Grenze bestätigt, ChatGPT streicht „Confidence hoch → automatisch"), kein ML/Embeddings/LLM im Importpfad, Rule Mining Priorität 1, `plan_document_segments` als Sample-Store, gemined-te Regeln werden Profilregeln v5, schlanke `recognition_feedback`-Tabelle. **ChatGPT-Korrekturen übernommen:** Profil ≠ Quelle (projektlokale Config, code-verifiziert), hierarchisches Evidenz-Backoff Projekt → Profil-Lineage (`profileLineageId` vererbt bei Kopie) → globales Lexikon (nur Tokenformen, nie Werte/IDs). **Herbert-Entscheidungen:** Backoff-Modell ja · Mining-Katalog schmal (AtPosition/Prefix/Suffix/Shape + ExactToken nur token-grenzen-basiert; OrderPair/Count warten auf FieldExtractionRule) · Alias-Stufe zurück in Roadmap (eigene Stufe, BPM-109.06) |
